...
path_if = "seap_dataset/contract_awards_IF/"
...
df_raw = load_data(os.path.join(path_if, current_file))
df_all_contracts = load_data(path_contracts) 
...
if not df_raw.empty:
    ...
    if not df.empty:
        ...
        st.dataframe(df.sort_values(by='anomaly_score'), width='stretch')
